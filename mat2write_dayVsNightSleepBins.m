...
close all;
primedir = 'C:\Users\Windows 10\Dropbox\Sehgal Lab\Sensory DAM\KIR ant glue 25deg';
xl2read = 'R53B06 mat2read.xlsx';
...
for(zti = 1:numZTbins),
    if(numel(ztBinBounds)==1),
        output = strrep(xl2read,'mat2read.xlsx',['_24hrs.xlsx']);
    else,
        output = strrep(xl2read,'mat2read.xlsx',['_ZT' num2str(ztBinBounds(zti)) 'to' num2str(ztBinBounds(zti+1)) '.xlsx']);
    end;
    prevRow = 2;
    cd(primedir);
...
%                 end;
                %             try,
                [durationHist, allDurations,~,numBouts] = durationHistogram(thisFlyBinary, durationMinuteBins);
